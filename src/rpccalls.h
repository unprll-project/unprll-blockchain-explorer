...
{
// declare struct in unprll's cryptonote namespace.
// unprll should provide definition for this,
...
public:
    ...
    rpccalls(string _daemon_url = "http:://127.0.0.1:21150",
             uint64_t _timeout = 200000);

    bool
    connect_to_unprll_daemon();
    ...
    get_alt_blocks(vector<string>& alt_blocks_hashes)
    {
        ...
        if (r)
        {
            ...
            if (!err.empty())
            {
                cerr << "Error connecting to Unprll daemon due to "
                     << err << endl;
                return false;
            }
        }
        ...
        {
            cerr << "Error connecting to Unprll daemon at "
                 << daemon_url << endl;
            return false;
        }
