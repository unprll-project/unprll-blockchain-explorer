...
struct tx_details
{
    ...
    // public keys and ull amount of outputs
    ...
    get_mstch_map() const
    {
        ...
        const double& ull_amount = ULL_AMOUNT(fee);
        ...
        {
            double payed_for_kB = ull_amount / tx_size;

            mixin_str        = std::to_string(mixin_no);
            fee_str          = fmt::format("{:0.6f}", ull_amount);
            fee_short_str    = fmt::format("{:0.4f}", ull_amount);
            fee_micro_str    = fmt::format("{:04.0f}" , ull_amount * 1e6);
        ...
        mstch::map txd_map {
                ...
                {"sum_inputs"        , ull_amount_to_str(ull_inputs , "{:0.6f}")},
                {"sum_outputs"       , ull_amount_to_str(ull_outputs, "{:0.6f}")},
                {"sum_inputs_short"  , ull_amount_to_str(ull_inputs , "{:0.3f}")},
                {"sum_outputs_short" , ull_amount_to_str(ull_outputs, "{:0.3f}")},
          ...
          tx_context_cache(1000)
{
    ...
    template_file["css_styles"]      = ullblocks::read(TMPL_CSS_STYLES);
    template_file["header"]          = ullblocks::read(TMPL_HEADER);
    template_file["footer"]          = get_footer();
    template_file["index2"]          = get_full_page(ullblocks::read(TMPL_INDEX2));
    template_file["mempool"]         = ullblocks::read(TMPL_MEMPOOL);
    template_file["altblocks"]       = get_full_page(ullblocks::read(TMPL_ALTBLOCKS));
    template_file["mempool_error"]   = ullblocks::read(TMPL_MEMPOOL_ERROR);
    template_file["mempool_full"]    = get_full_page(template_file["mempool"]);
    template_file["block"]           = get_full_page(ullblocks::read(TMPL_BLOCK));
    template_file["tx"]              = get_full_page(ullblocks::read(TMPL_TX));
    template_file["my_outputs"]      = get_full_page(ullblocks::read(TMPL_MY_OUTPUTS));
    template_file["rawtx"]           = get_full_page(ullblocks::read(TMPL_MY_RAWTX));
    template_file["checkrawtx"]      = get_full_page(ullblocks::read(TMPL_MY_CHECKRAWTX));
    template_file["pushrawtx"]       = get_full_page(ullblocks::read(TMPL_MY_PUSHRAWTX));
    template_file["rawkeyimgs"]      = get_full_page(ullblocks::read(TMPL_MY_RAWKEYIMGS));
    template_file["rawoutputkeys"]   = get_full_page(ullblocks::read(TMPL_MY_RAWOUTPUTKEYS));
    template_file["checkrawkeyimgs"] = get_full_page(ullblocks::read(TMPL_MY_CHECKRAWKEYIMGS));
    template_file["checkoutputkeys"] = get_full_page(ullblocks::read(TMPL_MY_CHECKRAWOUTPUTKEYS));
    template_file["address"]         = get_full_page(ullblocks::read(TMPL_ADDRESS));
    template_file["search_results"]  = get_full_page(ullblocks::read(TMPL_SEARCH_RESULTS));
    template_file["tx_details"]      = ullblocks::read(string(TMPL_PARIALS_DIR) + "/tx_details.html");
    template_file["tx_table_header"] = ullblocks::read(string(TMPL_PARIALS_DIR) + "/tx_table_header.html");
    template_file["tx_table_row"]    = ullblocks::read(string(TMPL_PARIALS_DIR) + "/tx_table_row.html");

    if (enable_js) {
        // JavaScript files
        template_file["jquery.min.js"]   = ullblocks::read(JS_JQUERY);
        template_file["crc32.js"]        = ullblocks::read(JS_CRC32);
        template_file["crypto.js"]       = ullblocks::read(JS_CRYPTO);
        template_file["cn_util.js"]      = ullblocks::read(JS_CNUTIL);
        template_file["base58.js"]       = ullblocks::read(JS_BASE58);
        template_file["nacl-fast-cn.js"] = ullblocks::read(JS_NACLFAST);
        template_file["sha3.js"]         = ullblocks::read(JS_SHA3);
        template_file["config.js"]       = ullblocks::read(JS_CONFIG);
        template_file["biginteger.js"]   = ullblocks::read(JS_BIGINT);
...
index2(uint64_t page_no = 0, bool refresh_page = false)
{
    ...
    std::future<json> network_info_ftr = std::async(std::launch::async, [&]
    {
        ...
        get_unprll_network_info(j_info);
    ...
    mstch::map context {
            ...
            {"height"                   , height},
            {"server_timestamp"         , ullblocks::timestamp_to_str_gm(local_copy_server_timestamp)},
    ...
    //double blk_size_median = ullblocks::calc_median(blk_sizes.begin(), blk_sizes.end());
    ...
    if (CurrentBlockchainStatus::is_thread_running())
    {
        ...
        string emission_coinbase = ull_amount_to_str(current_values.coinbase, "{:0.3f}");
        string emission_fee      = ull_amount_to_str(current_values.fee, "{:0.3f}");
...
mempool(bool add_header_and_footer = false, uint64_t no_of_mempool_tx = 25)
{
    ...
    for (size_t i = 0; i < no_of_mempool_tx; ++i)
    {
        ...
        txs.push_back(mstch::map {
                ...
                {"ull_inputs"      , mempool_tx.ull_inputs_str},
                {"ull_outputs"     , mempool_tx.ull_outputs_str},
...
show_block(uint64_t _blk_height)
{
    ...
    string blk_timestamp = ullblocks::timestamp_to_str_gm(blk.timestamp);
    ...
    for (size_t i = 0; i < blk.tx_hashes.size(); ++i)
    {
        ...
        //string mixin_times_scale = ullblocks::timestamps_time_scale(mixin_timestamps,
        //                                                        server_timestamp);
    ...
    context["sum_fees"]
            = ullblocks::ull_amount_to_str(sum_fees, "{:0.6f}", false);

    // get ull in the block reward
    context["blk_reward"]
            = ullblocks::ull_amount_to_str(txd_coinbase.ull_outputs - sum_fees, "{:0.6f}");
...
show_block(string _blk_hash)
{
    ...
    if (!ullblocks::parse_str_secret_key(_blk_hash, blk_hash))
    {
...
show_tx(string tx_hash_str, uint16_t with_ring_signatures = 0)
{
    ...
    if (!ullblocks::parse_str_secret_key(tx_hash_str, tx_hash))
    {
    ...
    if (!mcore->get_tx(tx_hash, tx))
    {
        ...
        if (!found_txs.empty())
        {
            ...
            blk_timestamp = ullblocks::timestamp_to_str_gm(tx_recieve_timestamp);
                ...
                bool tx_prove = false)
{
    ...
    if (ull_address_str.empty())
    {
        return string("Unprll address not provided!");
    }
    ...
    // parse string representing given unprll address
    cryptonote::address_parse_info address_info;

    if (!ullblocks::parse_str_address(ull_address_str,  address_info, nettype))
    {
        cerr << "Cant parse string address: " << ull_address_str << endl;
        return string("Cant parse ull address: " + ull_address_str);
    }
    ...
    if (!ullblocks::parse_str_secret_key(viewkey_str, multiple_tx_secret_keys))
    {
...
//        if (!ullblocks::parse_str_secret_key(spend_key_str, prv_spend_key))
//        {
...
//        }
    ...
    if (tx_blk_found)
    {
        ...
        blk_timestamp = ullblocks::timestamp_to_str_gm(blk.timestamp);

        tx_blk_height_str = std::to_string(tx_blk_height);
    }

    // payments id. encrypted (payment_id8)
    ...
    mstch::map context {
            ...
            {"ull_address"          , ull_address_str},
            {"viewkey"              , viewkey_str_partial},
            {"tx_pub_key"           , pod_to_hex(txd.pk)},
            {"blk_height"           , tx_blk_height_str},
            {"tx_size"              , fmt::format("{:0.4f}",
                                                  static_cast<double>(txd.size) / 1024.0)},
            {"tx_fee"               , ullblocks::ull_amount_to_str(txd.fee, "{:0.12f}", true)},
            {"blk_timestamp"        , blk_timestamp},
    ...
    string server_time_str = ullblocks::timestamp_to_str_gm(server_timestamp, "%F");
    ...
    for (pair<txout_to_key, uint64_t>& outp: txd.output_pub_keys)
    {
        ...
        // if someone had sent us some ull.
        ...
        outputs.push_back(mstch::map {
                ...
                {"amount"                , ullblocks::ull_amount_to_str(outp.second)},
    ...
    // to hold sum of ull in matched mixins, those that
    // perfectly match mixin public key with outputs in mixn_tx.
    uint64_t sum_mixin_ull {0};
    ...
    // we get number of all posibilities, and their total ull amount
    ...
    for (const txin_to_key& in_key: input_key_imgs)
    {
        ...
        inputs.push_back(mstch::map{
                ...
                {"key_image_amount", ullblocks::ull_amount_to_str(in_key.amount)},
        ...
        for (const uint64_t& abs_offset: absolute_offsets)
        {
            ...
            public_key mixin_tx_pub_key
                    = ullblocks::get_tx_pub_key_from_received_outs(mixin_tx);
            ...
            output_pub_keys = ullblocks::get_ouputs_tuple(mixin_tx);
...
//                        }
                ...
                found_outputs.push_back(mstch::map {
                        ...
                        {"amount"          , ullblocks::ull_amount_to_str(amount)}
                });
                ...
                if (mine_output)
                {
                    ...
                    // increase sum_mixin_ull only when
...
//
//                        if (!ullblocks::generate_key_image(derivation,
...
//                                << endl;
    ...
    context["found_our_outputs"] = (sum_ull > 0);
    context["sum_ull"]           = ullblocks::ull_amount_to_str(sum_ull);

    context.emplace("inputs", inputs);

    context["show_inputs"]   = show_key_images;
    context["inputs_no"]     = static_cast<uint64_t>(inputs.size());
    context["sum_mixin_ull"] = ullblocks::ull_amount_to_str(
            sum_mixin_ull, "{:0.12f}", false);
    ...
    // useful for unit testing as it provides total ull sum
    // of possible mixins
    ...
    if (sum_mixin_ull > (sum_ull + txd.fee)
        && no_of_matched_mixins == inputs.size())
    {
        //                  (outcoming    - incoming) - fee
        possible_spending = (sum_mixin_ull - sum_ull) - txd.fee;
    }

    context["possible_spending"] = ullblocks::ull_amount_to_str(
           ...
           string domain)
{
    ...
    return show_my_outputs(tx_hash_str, ull_address_str,
...
show_checkrawtx(string raw_tx_data, string action)
{
    ...
    string data_prefix = ullblocks::make_printable(decoded_raw_tx_data.substr(0, magiclen));
    ...
    if (unsigned_tx_given)
    {
        ...
        if (r)
        {
            ...
            for (const ::tools::wallet2::tx_construction_data& tx_cd: exported_txs.txes)
            {
                ...
                get_payment_id(tx_cd.extra, payment_id8);
                ...
                mstch::map tx_cd_data {
                        ...
                        {"change_amount"      , ullblocks::ull_amount_to_str(tx_change.amount)},
                        {"has_payment_id8"    , (payment_id8 != null_hash8)},
                        {"payment_id8"        , pid8_str},
                };
                ...
                for (const tx_destination_entry& a_dest: tx_cd.splitted_dsts)
                {
                    mstch::map dest_info {
                            ...
                            {"dest_amount"   , ullblocks::ull_amount_to_str(a_dest.amount)}
                    };
                ...
                for (size_t i = 0; i < no_of_sources; ++i)
                {
                    ...
                    mstch::map single_dest_source {
                            {"output_amount"              , ullblocks::ull_amount_to_str(tx_source.amount)},
                ...
                tx_cd_data.insert({"sum_outputs_amounts" ,
                                   ullblocks::ull_amount_to_str(sum_outputs_amounts)});
                ...
                tx_cd_data["min_mix_time"]     = ullblocks::timestamp_to_str_gm(min_mix_timestamp);
                tx_cd_data["max_mix_time"]     = ullblocks::timestamp_to_str_gm(max_mix_timestamp);
    ...
    else
    {
        ...
        if (strncmp(decoded_raw_tx_data.c_str(), SIGNED_TX_PREFIX, magiclen) != 0)
        {
            ...
            // used in myunprll and openunprll projects.
...
//                serialization::serialize(ba, tx_from_blob);
        ...
        for (tools::wallet2::pending_tx& ptx: ptxs)
        {
            ...
            if (ptx.construction_data.change_dts.amount > 0)
            {
                destination_addresses.push_back(
                        mstch::map {
                                ...
                                {"dest_amount"    ,
                                        ullblocks::ull_amount_to_str(ptx.construction_data.change_dts.amount)},
                                {"is_this_change" , true}
                        }
                );
            ...
            tx_context["outputs_ull_sum"] = ullblocks::ull_amount_to_str(outputs_ull_sum);
            ...
            for (size_t i = 0; i < outputs.size(); ++i)
            {
                ...
                if (parse_amount(output_amount, out_amount_str))
                {
                    if (output_amount == 0)
                    {
                        out_amount_str = ullblocks::ull_amount_to_str(real_ammounts.at(i));
                    }
                }
            }
            ...
            for (const tx_source_entry&  tx_source: ptx.construction_data.sources)
            {
                ...
                inputs_ull_sum += tx_source.amount;
            }

            // mark that we have signed tx data for use in mstch
            tx_context["have_raw_tx"] = true;

            // provide total mount of inputs ull
            tx_context["inputs_ull_sum"] = ullblocks::ull_amount_to_str(inputs_ull_sum);
            ...
            for (mstch::node& input_node: inputs)
            {
                ...
                amount = ullblocks::ull_amount_to_str(real_amounts.at(input_idx));
...
show_pushrawtx(string raw_tx_data, string action)
{
    ...
    // if failed, treat raw_tx_data as base64 encoding of signed_unprll_tx
    else
    {
        string decoded_raw_tx_data = epee::string_encoding::base64_decode(raw_tx_data);
...
show_checkrawkeyimgs(string raw_data, string viewkey_str)
{
    ...
    if (!ullblocks::parse_str_secret_key(viewkey_str, prv_view_key))
    {
    ...
    string data_prefix = ullblocks::make_printable(decoded_raw_data.substr(0, magiclen));
    ...
    decoded_raw_data = ullblocks::decrypt(
    ...
    // get ull address stored in this key image file
    const account_public_address* ull_address =
            reinterpret_cast<const account_public_address*>(
                    decoded_raw_data.data());

    address_parse_info address_info {*ull_address, false};


    context.insert({"address"        , REMOVE_HASH_BRACKETS(
            ullblocks::print_address(address_info, nettype))});
    context.insert({"viewkey"        , REMOVE_HASH_BRACKETS(
            fmt::format("{:s}", prv_view_key))});
    context.insert({"has_total_ull"  , false});
    context.insert({"total_ull"      , string{}});
    ...
    for (size_t n = 0; n < no_key_images; ++n)
    {
        ...
        mstch::map key_img_info {
                ...
                {"address"             , ullblocks::print_address(
...
show_checkcheckrawoutput(string raw_data, string viewkey_str)
{
    ...
    address_parse_info address_info {*ull_address, false, false, crypto::null_hash8};
    ...
    for (const tools::wallet2::transfer_details& td: outputs)
    {
        ...
        if (td.is_rct())
        {
            ...
            public_key tx_pub_key = ullblocks::get_tx_pub_key_from_received_outs(tx);
        ...
        mstch::map output_info {
                ...
                {"amount"              , ullblocks::ull_amount_to_str(ull_amount)},
                {"tx_hash"             , REMOVE_HASH_BRACKETS(fmt::format("{:s}", td.m_txid))},
                {"timestamp"           , ullblocks::timestamp_to_str_gm(blk_timestamp)},
    ...
    if (total_ull > 0)
    {
        context["has_total_ull"] = true;
        context["total_ull"] = ullblocks::ull_amount_to_str(total_ull);
    }
...
search(string search_text)
{
    ...
    // check if unprll address is given based on its length
    ...
    if (search_str_length == 95)
    {
        ...
        if (!ullblocks::parse_str_address(search_text, address_info, nettype_addr))
        {
    ...
    // check if integrated unprll address is given based on its length
...
show_address_details(const address_parse_info& address_info, cryptonote::network_type nettype = cryptonote::network_type::MAINNET)
{
    ...
    string address_str      = ullblocks::print_address(address_info, nettype);
    ...
    mstch::map context {
            {"ull_address"        , REMOVE_HASH_BRACKETS(address_str)},
    ...
    for (const pair<string, vector<string>>& found_txs: all_possible_tx_hashes)
    {
        ...
        if (!found_txs.second.empty())
        {
            ...
            for (const string& tx_hash: found_txs.second)
            {
                ...
                txd_map.insert({"timestamp", ullblocks::timestamp_to_str_gm(blk_timestamp)});
...
json_transaction(string tx_hash_str)
{
    ...
    string blk_timestamp_utc = ullblocks::timestamp_to_str_gm(tx_timestamp);
...
json_detailedtransaction(string tx_hash_str)
{
    ...
    tx_context.erase("inputs_ull_sum_not_zero");
...
json_block(string block_no_or_hash)
{
    ...
    else if (block_no_or_hash.length() == 64)
    {
        ...
        if (!ullblocks::parse_str_secret_key(block_no_or_hash, blk_hash))
        {
    ...
    j_data = json {
            ...
            {"timestamp_utc" , ullblocks::timestamp_to_str_gm(blk.timestamp)},
            {"block_height"  , block_height},
            {"size"          , blk_size},
            {"txs"           , j_txs},
             ...
             bool tx_prove = false)
{
    ...
    {
        j_response["status"]  = "error";
        j_response["message"] = "Unprll address not provided";
        return j_response;
    }
    ...
    if (!ullblocks::parse_str_address(address_str,  address_info, nettype))
    {
        j_response["status"]  = "error";
        j_response["message"] = "Cant parse unprll address: " + address_str;
        return j_response;
...
json_networkinfo()
{
    ...
    if (!get_unprll_network_info(j_info))
    {
        j_response["status"]  = "error";
        j_response["message"] = "Cant get unprll network info";
        return j_response;
    }
...
json_emission()
{
    ...
    else
    {
        ...
        string emission_blk_no   = std::to_string(current_values.blk_no - 1);
        string emission_coinbase = ull_amount_to_str(current_values.coinbase, "{:0.3f}");
        string emission_fee      = ull_amount_to_str(current_values.fee, "{:0.4f}", false);
...
json_version()
{
    ...
    j_data = json {
            ...
            {"unprll_version_full" , string {UNPRLL_VERSION_FULL}},
...
get_tx_json(const transaction& tx, const tx_details& txd)
{
    ...
    json j_tx {
            ...
            {"mixin"       , txd.mixin_no},
            {"tx_size"     , txd.size},
            {"ull_outputs" , txd.ull_outputs},
            {"ull_inputs"  , txd.ull_inputs},
...
construct_tx_context(transaction tx, uint16_t with_ring_signatures = 0)
{
    ...
    double payed_for_kB = ULL_AMOUNT(txd.fee) / tx_size;
    ...
    mstch::map context {
            ...
            {"tx_size"               , fmt::format("{:0.4f}", tx_size)},
            {"tx_fee"                , ullblocks::ull_amount_to_str(txd.fee, "{:0.12f}", false)},
            {"tx_fee_micro"          , ullblocks::ull_amount_to_str(txd.fee*1e6, "{:0.4f}", false)},
    ...
    for (const txin_to_key &in_key: txd.input_key_imgs)
    {
        ...
        inputs.push_back(mstch::map {
                ...
                {"amount"       , ullblocks::ull_amount_to_str(in_key.amount)},
        ...
        for (const uint64_t& i: absolute_offsets)
        {
            ...
            if (detailed_view)
            {
                ...
                mixins.push_back(mstch::map {
                        ...
                        {"mix_timestamp",  ullblocks::timestamp_to_str_gm(blk.timestamp)},
    ...
    if (detailed_view)
    {
        ...
        context["min_mix_time"]     = ullblocks::timestamp_to_str_gm(min_mix_timestamp);
        context["max_mix_time"]     = ullblocks::timestamp_to_str_gm(max_mix_timestamp);
    ...
    context["inputs_ull_sum_not_zero"]  = (inputs_ull_sum > 0);
    context["inputs_ull_sum"]           = ullblocks::ull_amount_to_str(inputs_ull_sum);
    ...
    context["outputs_ull_sum"] = ullblocks::ull_amount_to_str(outputs_ull_sum);
...
)
{
    ...
    {
        // get mixins in time scale for visual representation
        pair<string, double> mixin_times_scale = ullblocks::timestamps_time_scale(
                mixn_timestamps,
                max_mix_timestamp,
                170,
                min_mix_timestamp);
               ...
               uint64_t bc_height = 0)
{
    ...
    // https://github.com/unprll-project/unprll/pull/1358/commits/7abfc5474c0f86e16c405f154570310468b635c2
    txd.pk = ullblocks::get_tx_pub_key_from_received_outs(tx);
    txd.additional_pks = cryptonote::get_additional_tx_pub_keys_from_extra(tx);


    // sum ull in inputs and ouputs in the given tx
    const array<uint64_t, 4>& sum_data = summary_of_in_out_rct(
            tx, txd.output_pub_keys, txd.input_key_imgs);

    txd.ull_outputs       = sum_data[0];
    txd.ull_inputs        = sum_data[1];
    ...
    get_payment_id(tx, txd.payment_id8);
    ...
    // get unlock delta
    txd.unlock_delta = tx.unlock_delta;
...
bool
get_unprll_network_info(json& j_info)
{
...
get_footer()
{
    ...
    string footer_html = mstch::render(ullblocks::read(TMPL_FOOTER), footer_context);
