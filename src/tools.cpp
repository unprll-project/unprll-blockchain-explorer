...
/**
* Parse unprll address in a string form into
...
/**
* Return string representation of unprll address
*/
string
...
{
    // default path to unprll folder
    // on linux this is /home/<username>/.unprll
    string default_unprll_dir = tools::get_default_data_dir();

    if (nettype == cryptonote::network_type::TESTNET)
        default_unprll_dir += "/testnet";
    if (nettype == cryptonote::network_type::STAGENET)
        default_unprll_dir += "/stagenet";


    // the default folder of the lmdb blockchain database
    // is therefore as follows
    return default_unprll_dir + string("/lmdb");
}
...
{
    // the default folder of the lmdb blockchain database
    string default_lmdb_dir   = ullblocks::get_default_lmdb_folder(nettype);
    ...
    blockchain_path = ullblocks::remove_trailing_path_separator(blockchain_path);
...
{
    pair<uint64_t, uint64_t> sum_ull {0, 0};
    ...
    {
        sum_ull.first += vout["amount"].get<uint64_t>();
        ++sum_ull.second;
    }
...
{
    pair<uint64_t, uint64_t> sum_ull {0ULL, 0ULL};
        ...
        vector<txin_to_key>& input_key_imgs)
{
    ...
    uint64_t ull_outputs       {0};
    uint64_t ull_inputs        {0};
    uint64_t mixin_no          {0};
    ...
    for (size_t i = 0; i < input_no; ++i)
    {
        ...
        ull_inputs += tx_in_to_key.amount;
    ...
    return {ull_outputs, ull_inputs, mixin_no, num_nonrct_inputs};
};
...
summary_of_in_out_rct(const json& _json)
{
    ...
    for (const json& vout: _json["vout"])
    {
        ull_outputs += vout["amount"].get<uint64_t>();
    }
    ...
    return {ull_outputs, ull_inputs, no_outputs, no_inputs, mixin_no, num_nonrct_inputs};
};
...
sum_money_in_inputs(const string& json_str)
{
    ...
    for (json& vin: j["vin"])
    {
        sum_ull.first += vin["key"]["amount"].get<uint64_t>();
        ++sum_ull.second;
    }
...
{
    array<uint64_t, 2> sum_ull;

    sum_ull[0] = sum_money_in_inputs(tx);
    sum_ull[1] = sum_money_in_outputs(tx);
...
get_payment_id(const transaction& tx,
               crypto::hash8& payment_id8)
{
    return get_payment_id(tx.extra, payment_id8);
}
