...
MempoolStatus::read_mempool()
{
    ...
    for (size_t i = 0; i < mempool_tx_info.size(); ++i)
    {
        ...
        double payed_for_kB = ULL_AMOUNT(_tx_info.fee) / tx_size;
        ...
        last_tx.fee_str          = ullblocks::ull_amount_to_str(_tx_info.fee, "{:0.4f}", false);
        last_tx.fee_micro_str    = ullblocks::ull_amount_to_str(_tx_info.fee*1.0e6, "{:04.0f}", false);
        last_tx.payed_for_kB_str = fmt::format("{:0.4f}", payed_for_kB);
        last_tx.payed_for_kB_micro_str = fmt::format("{:04.0f}", payed_for_kB*1e6);
        last_tx.ull_inputs_str   = ullblocks::ull_amount_to_str(last_tx.sum_inputs , "{:0.3f}");
        last_tx.ull_outputs_str  = ullblocks::ull_amount_to_str(last_tx.sum_outputs, "{:0.3f}");
        last_tx.timestamp_str    = ullblocks::timestamp_to_str_gm(_tx_info.receive_time);
...
bf::path MempoolStatus::blockchain_path {"/home/mwo/.unprll/lmdb"};
string MempoolStatus::daemon_url {"http:://127.0.0.1:21150"};
...
ullblocks::MicroCore*  MempoolStatus::mcore {nullptr};
