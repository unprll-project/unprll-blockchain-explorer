...
struct outputs_visitor
{
    ...
    bool handle_output(uint16_t unlock_delta, const crypto::public_key &pubkey)
    {
...
static
string
ull_amount_to_str(const uint64_t& ull_amount,
                  ...
                  bool zero_to_question_mark=true)
{
    ...
    {
        amount_str = fmt::format(_format, ULL_AMOUNT(ull_amount));
    }
    else
    {
        if (ull_amount > 0 && zero_to_question_mark == true)
        {
            amount_str = fmt::format(_format, ULL_AMOUNT(ull_amount));
        }
    }
