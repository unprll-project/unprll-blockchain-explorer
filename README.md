# Unprll Blockchain Explorer

The official Unprll Blockchain Explorer. Based off onion-monero-blockchain-explorer

## Features

The key features of the Unprll Blockchain Explorer are:

 - No cookies, no analytical trackers, no images
 - By default no JavaScript, but can be enabled for client side decoding and proving of transactions
 - Open source
 - Made completely in C++
 - Shows encrypted payments ID
 - Shows ring signatures
 - Shows transaction extra
 - Shows public components of Unprll addresses
 - Can decode which outputs and mixins belong to the given Unprll address and view key
 - Can prove that you sent Unprll to someone
 - Shows detailed information about ring members such as their age, timescale and their ring sizes
 - Shows number of amount output indices
 - Supports Unprll testnet and stagnet networks
 - Transaction checker and pusher to submit transactions online
 - Estimate possible spending based on address and view key
 - Can provide total circulating supply
 - Can decode encrypted payment IDs
 - Can decode outputs and prove transactions sent to sub-addresses

## Development branch

Current development branch:
 - https://github.com/unprll-project/unprll-blockchain-explorer/tree/devel

## Compilation on Ubuntu 16.04/18.04
##### Compile the latest Unprll version

Download and compile Unprll into your home folder:

```bash
# first install unprll dependencies
sudo apt update
...
Once Unprll is compiled, the explorer can be downloaded and compiled as follows:

```bash
# Go to home folder if still in ~/unprll
cd ~
# Clone the repository
git clone https://github.com/unprll-project/unprll-blockchain-explorer.git
# Enter the downloaded sourced code folder
cd unprll-blockchain-explorer
# Make a build folder and enter it
mkdir build && cd build
# Create the Makefile
cmake ..
# Alternatively you can use: cmake -DUNPRLL_DIR=/path/to/unprll_folder ..
# if unprll is not in ~/unprll
#
# You can also build with ASAN (sanitizers), for example
...
By default it will look for the blockchain in its default location i.e., `~/.unprll/lmdb`.
You can use the `-b` option if its in a different location.

For example:
```bash
./ullblocks -b /home/mwo/non-default-unprll-location/lmdb/
```

Example output:
```bash
[mwo@arch unprll-blockchain-explorer]$ ./ullblocks
...
```
ullblocks, Unprll Blockchain Explorer:
  ...
  --enable-emission-monitor [=arg(=1)] (=0)
                                        enable Unprll total emission monitoring
                                        thread
  ...
  --ssl-key-file arg                    path to key file for ssl (https)
                                        functionality
  -d [ --daemon-url ] arg (=http:://127.0.0.1:21150)
                                        Unprll daemon url
```
...
alias ullblocksmainnet='~/unprll-blockchain-explorer/build/ullblocks    --port 8081 --testnet-url "http://139.162.32.245:8082" --enable-pusher --enable-emission-monitor'

# for testnet explorer
alias ullblockstestnet='~/unprll-blockchain-explorer/build/ullblocks -t --port 8082 --mainnet-url "http://139.162.32.245:8081" --enable-pusher --enable-emission-monitor'
```

These are aliases similar to those used for http://139.162.32.245:8081/ and http://139.162.32.245:8082/, respectively.

## Enable Unprll emission
Obtaining the current Unprll emission amount is not straight forward. Thus, by default it is disabled. To enable it use `--enable-emission-monitor` flag. For example:
```bash
ullblocks --enable-emission-monitor
```

This flag will enable the emission monitoring thread. When started, the thread will initially scan the entire blockchain, and calculate the cumulative emission based on each block. Since it is a separate thread, the explorer will work as usual during this time. Every 10000 blocks, the thread will save the current emission in a file, by default, in `~/.unprll/lmdb/emission_amount.txt`. For testnet or stagenet networks, it is `~/.unprll/testnet/lmdb/emission_amount.txt` or `~/.unprll/stagenet/lmdb/emission_amount.txt`. This file is used to avoid rescanning the entire blockchain whenever the explorer is restarted. When the explorer restarts, the thread will first check if `~/.unprll/lmdb/emission_amount.txt` is present, read its values, and continue from there if possible. Subsequently, only the initial use of the tread is time consuming. Once the thread scans the entire blockchain, it updates the emission amount using new blocks as they come. Since the explorer writes to this file, there can be only one instance (each) of it running for mainnet, testnet and stagenet. Thus, for example, you cant have two explorers for mainnet running at the same time, as they will be trying to write and read the same file at the same time, leading to unexpected results. Of course, having one instance for mainnet and one instance for testnet is fine, as they write to different files.

When the emission monitor is enabled, information about the current emission of coinbase and fees is displayed on the front page. For example:

```
Unprll emission (fees) is 14485540.430 (52545.373) as of 1313448 block
```

The values given, can be checked using the Unprll daemon's `print_coinbase_tx_sum` command. For example, for the above example: `print_coinbase_tx_sum 0 1313449`.

To disable the monitor, simply restart the explorer without `--enable-emission-monitor` flag.

## Enable JavaScript for decoding proving transactions
By default, decoding and proving transaction outputs are done on the server side. To do this on the client side (private view and transaction keys are not sent to the server), JavaScript-based decoding can be enabled:
```
ullblocks --enable-js
```
...
Having the `crt` and `key` files, run `ullblocks` in the following way:
```bash
./ullblocks --ssl-crt-file=/tmp/server.crt --ssl-key-file=/tmp/server.key
```

Note: Because we generated our own certificate, modern browsers will complain about it as they cant verify the certificate against any third party. Hence, for any practical use, get properly issued SSL certificates.

## JSON API
The explorer has a JSON API, which uses conventions defined by [JSend](https://labs.omniti.com/labs/jsend).
It is disabled by default. To enable it, use the `--enable-json-api` flag. For example:
```
./ullblocks --enable-json-api
```
...
Return number of newest mempool txs. For example: only 10.
...
```bash
# we use the official Unprll project donation address as an example
...
We use recipient's address (i.e. not our address from which we sent ULL to recipient).
...
```json
{
  "data": {
    ...
    "unprll_version_full": "0.1.0.0-1234abcd"
  },
  "status": "success"
}
```

API number is stored as a `uint32_t`. In this case `65536` represents major version 1 and minor version 0.
...
Return raw json block data, as represented in Unprll.
...
Return raw json tx data, as represented in Unprll.
